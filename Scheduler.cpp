...
//  CloudSim
//
//  Updated for Dynamic Consolidation-Based Scheduling Implementation
//
...
std::queue<TaskId_t> task_queue;

VMId_t Scheduler::FindOrCreateVM(MachineId_t machine_id, VMType_t vm_type, CPUType_t cpu_type) {
    // Check if a compatible VM already exists
    for (VMId_t vm_id : vms) {
        VMInfo_t vm_info = VM_GetInfo(vm_id);
        if (vm_info.machine_id == machine_id && vm_info.vm_type == vm_type) {
            return vm_id; // Return existing VM
        }
    }

    // Otherwise, create a new VM
    VMId_t new_vm_id = VM_Create(vm_type, cpu_type);
    VM_Attach(new_vm_id, machine_id);
    vms.push_back(new_vm_id);
    SimOutput("FindOrCreateVM(): Created new VM " + to_string(new_vm_id) + " on machine " + to_string(machine_id), 2);
    return new_vm_id;
}
...
void Scheduler::Init() {
    ...
    // Step 1: Organize machines into groups based on their attributes
    ...
    // Step 2: Initialize minimum required VMs for each group
    for (auto &group : machine_groups) {
        ...
        for (unsigned i = 0; i < init_vms; i++) {
            ...
            VMId_t new_vm = FindOrCreateVM(machine_id, default_vm_type, cpu_type);
            this->machines.push_back(machine_id);
        }

        // Put remaining machines in low-power state (S5)
...
void Scheduler::NewTask(Time_t now, TaskId_t task_id) {
    ...
    // Step 1: Check existing VMs on active machines
    ...
    // Step 2: Add a new VM to an already active machine
    ...
    // Step 3: Activate a new machine and create a VM
...
void Scheduler::TaskComplete(Time_t now, TaskId_t task_id) {
    ...
    for(int i = 0; i < size; ++i) {
        TaskId_t task = task_queue.front();
        task_queue.pop();
        TaskInfo_t task_info = GetTaskInfo(task);
        if(task_info.required_cpu == GetTaskInfo(task_id).required_cpu) {
            NewTask(now, task);
        } else {
...
void StateChangeComplete(Time_t time, MachineId_t machine_id) {
    ...
    if (machine_info.s_state == S0) {
        active_machines++;
        // for (VMId_t vm : vms) { // Dumb way to do it but it works
        //     VMInfo_t vm_info = VM_GetInfo(vm);

        //     if (vm_info.machine_id ==  machine_info.machine_id    // VM is on this machine
        //         machine_info.cpu == task_info.required_cpu &&     // CPU type matches
        //         vm_info.vm_type == task_info.required_vm &&       // VM type matches
        //         machine_info.memory_used + task_info.required_memory <= machine_info.memory_size // Memory fits
        //         ) {      
                
        //         for(TaskId_t task : task_queue) {
        //             if (machine_info.s_state == S0 &&                     // Machine is active
        //                 machine_info.cpu == task_info.required_cpu &&     // CPU type matches
        //                 vm_info.vm_type == task_info.required_vm &&       // VM type matches
        //                 machine_info.memory_used + task_info.required_memory <= machine_info.memory_size ) { // Memory fits
        //                 VM_AddTask(vm, task, GetTaskInfo(task).priority);
        //             }
        ...
        //     for(TaskId_t task : task_queue) {
        //         TaskInfo_t task_info = GetTaskInfo(task);
        //         if (machine_info.cpu == task_info.required_cpu &&     // CPU type matches
        //             machine_info.memory_used + task_info.required_memory + 8 <= machine_info.memory_size) { // Memory fits
        //             VMId_t new_vm = VM_Create(task_info.required_vm, task_info.required_cpu);
        //             VM_Attach(new_vm, machine_id);
        //             VM_AddTask(new_vm, task_id, task_info.priority);
        ...
        //             SimOutput("Scheduler::StateChangeComplete(): Added new VM " + to_string(new_vm) +
        //                     " to active machine " + to_string(machine_id) + " for task " + to_string(task_id), 2);
        //             return;
        //         }
        //     }
        ...
        for (size_t i = 0; i < size; ++i) {
            ...
            task_queue.pop();
            TaskInfo_t task_info = GetTaskInfo(task);
            Scheduler.NewTask(task_info.arrival, task);
